...
const checkAllowFile = (path) => {
    if ( !fs.statSync(path).isFile() ) { return false; }
    if ( !mime.getType(path).match( /^image\/.*/ )) { return false; }
    return true;
}

const changeViewFile = (path, window) => {
    //store.set({ viewingFile: path })
    if ( !checkAllowFile(path) ) { return; }
    const type = mime.getType(path)

    fs.readFile(path, (err, data) => {
        const dataUrl = 'data:' + type + ';base64,' + Buffer.from(data).toString('base64');
        window.webContents.send('viewImage', dataUrl);
    });
}
...
async function createWindow() {
    ...
    win.webContents.on("did-finish-load", () => {
        ...
        for (const arg of process.argv) {
            if ( !checkAllowFile(arg) ) { continue; }
            defaultImage = arg;
            break
        }

        //const viewingFile = defaultImage ?? store.get('viewingFile')
        const viewingFile = defaultImage;
        if (viewingFile) { changeViewFile(viewingFile, win); }
    });
    ...
    ipcMain.handle('dropFile', (ev, dropFilePath) => {
        ...
        changeViewFile(dropFilePath, win);
    })
